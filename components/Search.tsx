...
const Search = () => {
  ...
  return (
    <section className="w-full h-screen flex justify-center top-0 left-0 fixed bg-black/30 z-50">
      <div className="w-6/12 max-h-[600px] absolute top-28 rounded-xl overflow-hidden bg-green-black shadow-lg border border-teal/30">
        ...
        <div className="w-full h-full">
          <ul className="w-full max-h-[400px] overflow-scroll">
            {searchData.map((item) => (
              <li key={item.id} className="p-3 border-t border-teal/30">
                <Link href={`/detail/${item.media_type}/${item.id}`} className="flex gap-3 items-center">
